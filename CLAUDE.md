...
- **Knowledge FTS5**: `knowledge/knowledge.db` — 240K+ docs across 8 FTS tables via MCP `knowledge-fts` or CLI `tools/knowledge_indexer.py` (internal typed knowledge, external technique repos, ExploitDB, Nuclei, PoC-in-GitHub, Trickest CVE, web articles, triage objections). Auto-sync: `scripts/sync_poc_github.sh` (weekly cron, pulls nomi-sec/PoC-in-GitHub + reindex — see `scripts/README.md`)
- **Knowledge directory**: `knowledge/index.md` → `knowledge/challenges/` + `knowledge/techniques/`
- **Triage objections**: `knowledge/triage_objections/` — FTS5 indexed, surfaced through `routed_search(role="triager-sim", ...)`
...
- **Cross-Model**: Codex(GPT-5.5, plugin `codex@openai-codex`) — `/codex:review`, `/codex:adversarial-review`, `/codex:rescue` | Wrapper: `tools/codex_cross_review.sh`
...
GPT-5.5 via Codex plugin for cross-model verification at pipeline checkpoints:
