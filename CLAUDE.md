...
- **MCP (inventory=18, registered=8)**: gdb, pentest, pentest-thinking, context7, frida, ghidra, knowledge-fts, nuclei, codeql, semgrep, graphrag-security, lightpanda, browser-use, opendataloader-pdf, promptfoo, markitdown-mcp, gmail, playwright
- **MCP registration state** (2026-04-17, v13.5): `.mcp.json` registers 8 servers — codeql-mcp, context7, gmail, graphrag-security, knowledge-fts, nuclei-mcp, playwright, semgrep-mcp. The remaining 10 are plugin/OMC-provided, install-only, or doc-aspirational. Run `tools/integration_audit.py --write` for authoritative install↔registration↔doc mapping (outputs `docs/integration-gaps.md`).
