...
- **자동 품질 게이트 (v12 Kill Gate 포함)** -- 6개 파이프라인 스킬 + 2 Kill Gates(v12)가 OOS finding, 약한 PoC, 비현실적 위협 모델, AI 템플릿 언어를 제출 전 자동 차단
...
**6개 자동 파이프라인 스킬 + 2 Kill Gates (v12):**
...
| **exploiter** | PoC 개발, 품질 등급 분류 (Evidence Tier E1-E4) | Opus | PoC 스크립트 + 증거 |
| **triager-sim** | 적대적 트리아저 -- 3 모드: finding-viability (Gate 1), PoC-destruction (Gate 2), report-review | Sonnet/Opus | SUBMIT / STRENGTHEN / KILL |
...
| **threat-modeler** | 신뢰 경계 매핑, 역할 매트릭스, 상태 머신 및 불변식 추출 | Sonnet | `threat_model.md` |
| **workflow-auditor** | 비즈니스 워크플로우 상태 전이 매핑 및 이상 탐지 | Sonnet | `workflow_audit.md` |
| **patch-hunter** | 보안 커밋에서 불완전 수정 및 변종 취약점 탐색 | Sonnet | `patch_analysis.md` |
...
├── tools/                   # 파이프라인 도구
│   ├── bb_preflight.py      #   파이프라인 게이트 검증 (init, rules-check, coverage-check,
