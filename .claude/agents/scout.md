...
11. **Cloudflare/JS-rendered listing fetch** — For listings on Cloudflare-protected platforms (huntr bounties page, Intigriti KB, YWH help-center, Bugcrowd auth-gated) use `python3 tools/fetch.py <url>` (CLI wrapper — safe from any CWD) or `bb_preflight.py fetch-program`. Both auto-escalate urllib → FlareSolverr (localhost:8191) → firecrawl-py → (fallback to Playwright MCP if still blocked). Raw `curl` / `WebFetch(r.jina.ai)` on those platforms returns 6-line 403 challenge — zero value. jina remains OK for static blogs/hacktivity background only (Rule 5 verbatim exemption).
