...
| Grafana Labs | Intigriti | Grafana OSS (`github.com/grafana/grafana`) | **SUBMISSION READY (2026-04-26)** — Viewer-readable `/api/v1/provisioning/contact-points` leaks plaintext Teams/Webhook/OnCall/Pushover credentials while DingDing is redacted in the same response; incomplete-fix variant of CVE-2025-3415. Manual gates still pending: video PoC, ZeroGPT browser check, user demo, stress test. | `targets/detail/submission/cve2025-3415-variant/` |
...
| knowledge-fts | 240K+ 문서 BM25 검색 (8 FTS tables: typed internal docs, external technique repos, exploits/CVEs, web articles, triage objections) |
...
Key repos: ExploitDB (47K+), PoC-in-GitHub (18K+), nuclei-templates (12K+), Trickest CVE (155K+), PayloadsAllTheThings (70+ categories), protocol-vulns-index (460+ protocol docs), llm-wiki references.

### Knowledge DB Snapshot (2026-04-28)

Current live counts are generated by `python3 tools/knowledge_indexer.py stats`.

| Source | Docs | Table |
|--------|------|-------|
| Typed internal docs (techniques, scenarios, decisions, submissions, sources, protocol-vulns) | 583 | `techniques` |
| External technique repos | 6,214 | `external_techniques` |
| ExploitDB | 46,968 | `exploitdb` |
| Nuclei templates | 12,632 | `nuclei` |
| PoC-in-GitHub | 18,741 | `poc_github` |
| Trickest CVE | 155,741 | `trickest_cve` |
| Crawled web articles | 1 | `web_articles` |
| Triage objections / kill reasons | 11 | `triage_objections` |
| **Total indexed rows** | **240,891** | |

Tools:
- `tools/knowledge_indexer.py build` — rebuild all FTS tables
- `tools/knowledge_fetcher.py fetch <url>` — add crawled articles to `web_articles`
